...
CREATE_MAPS_LINK = ToolDefinition(
    ...
    description=(
        "Создать безопасную ссылку-навигатор на место по названию/адресу — текстовый "
        "поиск в Google Maps, без выдуманных координат/place-id. Работает для любого "
        "места, даже если точный адрес неизвестен. Как пользоваться (специфика "
        "запроса, что делать при неоднозначности) — см. общую инструкцию в промпте."
    ),
    parameters={
        "type": "object",
        "properties": {
