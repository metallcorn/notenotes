...
CREATE_CALENDAR_EVENT = ToolDefinition(
    ...
    description=(
        "Подготовить данные события для добавления в личный календарь пользователя "
        "(Google/Apple/Outlook) — своей интеграции с календарём пока нет, поэтому "
        "пользователь получит кнопку, которая одним кликом откроет системный диалог "
        "добавления события. Используй, когда пользователь просит создать заметку или "
        "напоминание про конкретное будущее событие с известной датой. Дату для "
        "start бери из того, что явно сказал пользователь, — не выдумывай и не "
        "угадывай, если она не названа."
    ),
    parameters={
        "type": "object",
        "properties": {
