...
def _build_prompt(existing_tag_names: list[str], today: str) -> str:
    ...
    return (
        ...
        'совпадение в тексте), "query": то, что нужно ввести в поиск карт, '
        "чтобы найти именно ЭТО место}. Если у адреса рядом в тексте есть "
        "название места/организации/компании (магазин, станция, клиника, "
        "ресторан и т.п.) — ОБЯЗАТЕЛЬНО добавь это название в НАЧАЛО query "
        "перед адресом (например, 'Okręgowa Stacja Kontroli Pojazdów OSKP "
        "Poznań, ul. Głuszyna 6, 61-333 Poznań', а не просто голый адрес) "
        "— иначе поиск на карте покажет случайную точку по адресу, а не "
        "нужное место с его карточкой/отзывами/часами работы. Если такого "
        "названия рядом нет — query это просто адрес. Если в исходном "
        ...
        '[{"text": "ул. Ленина, 15", "query": "Кофейня Пуля, ул. Ленина, '
        '15, Москва"}]}'
    )
