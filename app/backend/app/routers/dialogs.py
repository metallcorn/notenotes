...
TOOL_PROMPT_FRAGMENTS: dict[str, str] = {
    "search_base": (
        "Если search_base/get_note нашли заметку с material_type=\"ticket\" "
        ...
        "На ЛЮБУЮ заметку/список, найденные через search_base/get_note "
        "(не только билеты), фронтенд САМ рисует под твоим ответом кнопку "
        ...
        "(4) кнопка привязана к вызову search_base/get_note именно в ЭТОМ "
        "твоём ответе — если id заметки уже известен из предыдущих реплик "
        "этого диалога, но в текущем ходу ты не вызвал search_base/get_note "
        "заново, кнопка не появится.\n\n"
        ...
        "ВАЖНОЕ ПРАВИЛО, общее для обеих карточек (заметка и ссылки "
        "внутри неё): они появляются, ТОЛЬКО если search_base/get_note "
        "были вызваны именно в этом твоём ответе — не в одном из "
        "предыдущих ходов диалога. Если пользователь просит показать/"
        "прислать/повторить заметку, ссылки или билет, о которых уже шла "
        "речь раньше в этом диалоге — ты ОБЯЗАН вызвать get_note или "
        "search_base заново прямо сейчас, даже если сам факт и содержимое "
        "заметки тебе и так известны из контекста. Один лишний вызов "
        "тула дешевле, чем ответ без единой работающей ссылки — если "
        "сомневаешься, вызывай тул."
    ),
    "run_python": (
