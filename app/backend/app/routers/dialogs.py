...
SYSTEM_PROMPT_BASE = (
    ...
    "После create_note/create_list/update_note/append_to_note фронтенд сам "
    "рисует под твоим ответом кнопку «Открыть «название»»» — не пиши "
    "никакой свой текст-ссылку или картинку на созданную/изменённую "
    "заметку (ни markdown ![]()/[](), ни голое название с восклицательным "
    "знаком) — кнопка появится сама по результату тула, твой текст рядом с "
    "ней либо не нужен, либо не будет рабочей ссылкой. Одной фразы "
    "по сути хватает («Готово, заметка обновлена»).\n\n"
    "Если пользователь прислал файл (фото/документ) в этом диалоге — в его "
    "сообщении есть настоящая ссылка вида ![подпись](/api/uploads/<id>). "
    "Когда создаёшь или обновляешь заметку с этим файлом — вставляй в "
    "content ЭТУ ссылку буквально, как есть. НИКОГДА не вставляй "
    "data:...;base64,... — ты не можешь знать байты файла посимвольно, "
    "любая такая строка выдумана и не покажет реальную картинку.\n\n"
    "Реальный найденный случай: пользователь прислал фото документа, "
    "«Распознано в приложенном файле» дало полный дословный текст (включая "
    "ФИО принявшего заявление), а заметка, которую ты создал по просьбе "
    "«сохрани», получилась короче — часть строк (в том числе то самое ФИО) "
    "пропала, хотя реально была в распознанном тексте. Когда создаёшь или "
    "обновляешь заметку НА ОСНОВЕ распознанного текста фото/документа — "
    "переноси ВСЕ строки дословного текста («Текст на изображении») "
    "целиком, не сокращай и не выбирай «самое важное» по своему усмотрению: "
    "пользователь просил сохранить документ, а не краткое содержание. "
    "Единственное, что можно менять, — оформление (заголовки/абзацы), не "
    "содержание. Нужно именно сокращённо — пользователь попросит явно "
    "(«кратко», «главное»).\n\n"
    ...
    for _ in range(MAX_TOOL_ITERATIONS):
        ...
        for tc in real_tool_calls:
            if tc.name == "web_search":
                ...
                tc.name in ("create_note", "update_note")
                and tc.arguments.get("content") is not None
                and _DATA_URI_RE.search(str(tc.arguments["content"]))
            ):
                # Реальный найденный случай: пользователь прислал фото в
                # диалог (в его сообщении — настоящая ссылка вида
                # ![...](/api/uploads/<id>)), модель распознала содержимое,
                # но при создании/обновлении заметки вместо этой ссылки
                # сама СОЧИНИЛА data:image/...;base64,... — правдоподобный
                # на вид, но не настоящие байты файла (модель физически не
                # может знать их посимвольно). Итог — фото "потерялось": в
                # заметке мусорный текст вместо картинки, а CLAUDE.md прямо
                # запрещает base64 для картинок (только отдельный файл по
                # ссылке). Тот же приём, что и у _missing_attachments_on_update
                # выше — ловим на уровне кода, не полагаемся на промпт.
                result = {
                    "error": (
                        "В content — data:...;base64,... вместо ссылки на файл. Это НЕ настоящее "
                        "содержимое фото — ты не можешь знать байты файла посимвольно, только "
                        "выдумал похожую строку. Если в этом диалоге пользователь прислал файл, "
                        "в его сообщении есть настоящая ссылка вида ![...](/api/uploads/<id>) — "
                        "используй её буквально, как есть в content (или как в исходном content "
                        "заметки при update_note). Вызови снова без base64."
                    )
                }
            else:
                result = await dispatch(tc.name, ctx, tc.arguments, disabled=disabled_tool_names)
